...
namespace {
    ...
    void rename_temporary(size_t limit) {
        for (size_t i = 0; i < limit; ++i) {
            rename(temporary_prefix + std::to_string(i), prefix + std::to_string(i));
        }
    }
}
...
external_sort(const std::string &file_name, unsigned long  memory_size, unsigned long block_size, Comp comp) {
    ...
    std::vector<std::string> names = split_and_sort(file_name, block_size, comp);

    auto iter = names.begin();
    size_t count = 1;
    const size_t blocks_count = (memory_size < 64 * 1024 * 1024 ? 64 * 1024 * 1024 : memory_size) / block_size - 1;
    std::vector<T> buffer;
